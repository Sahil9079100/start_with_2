/**
 * WORKDAY STEP 4: Sort Resumes by Job Description Match
 * 
 * This worker evaluates each candidate's resume against the job description
 * using AI and assigns match scores for Workday integration.
 * 
 * What it does:
...
/**
 * Main worker function to sort Workday candidates by job match
 * 
 * @param {string} interviewId - MongoDB ObjectId of the interview
 */
export const workday_sortResumes = async (interviewId) => {
    let interview;
    let workdayData;

    try {
        console.log(`[Workday Sort] Starting for interview: ${interviewId}`);
        ...
        // 2️⃣ Fetch WorkdayData for logging
        ...
        if (!candidates || candidates.length === 0) {
            console.log(`[Workday Sort] No unsorted candidates found for interview: ${interviewId}`);
        ...
        console.log(`[Workday Sort] Processing ${candidates.length} candidates for matching...`);
        ...
        const tasks = candidates.map((candidate, idx) => sem.run(async () => {
            ...
            if (!candidate.resumeSummary || candidate.resumeSummary.length < 50) {
                console.log(`[Workday Sort] Skipping ${candidate.email}: no resume text`);
            ...
            try {
                ...
                console.error(`[Workday Sort] Error sorting ${candidate.email}:`, candidateError.message);
                candidate.isResumeSorted = true;
                candidate.matchLevel = "Unqualified";
                candidate.matchScore = 0;
        ...
        interview.status = "workday_sorting_complete";
        ...
        console.log(`[Workday Sort] Completed for interview: ${interviewId}`);
        console.log(`[Workday Sort] Success: ${successCount}, Failed: ${failedCount}`);

    } catch (error) {
        console.error("[Workday Sort] Error:", error);

        // Update interview status
        if (interview) {
            interview.currentStatus = "FAILED";
            interview.logs.push({
                message: `Workday sorting failed: ${error.message}`,
                level: "error"
            });
            await interview.save();
