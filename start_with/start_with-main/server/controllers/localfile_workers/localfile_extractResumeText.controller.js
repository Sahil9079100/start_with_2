/**
 * LOCALFILE STEP 3: Extract Text from Resume URLs (OCR)
 * 
 * This worker extracts text from candidate resumes for LocalFile integration.
...
/**
 * Main worker function to extract text from LocalFile candidate resumes
 * 
 * @param {string} interviewId - MongoDB ObjectId of the interview
 */
export const localfile_extractResumeText = async (interviewId) => {
    let interview;
    let localFileData;

    try {
        console.log(`[LocalFile OCR] Starting for interview: ${interviewId}`);
        ...
        if (!candidates || candidates.length === 0) {
            console.log(`[LocalFile OCR] No unscanned candidates found for interview: ${interviewId}`);
        ...
        console.log(`[LocalFile OCR] Processing ${candidates.length} candidates`);
        ...
        for (let i = 0; i < totalCandidates; i++) {
            ...
            try {
                ...
                if (!candidate.resumeUrl || candidate.resumeUrl === "none" || candidate.resumeUrl.trim() === '') {
                    console.log(`[LocalFile OCR] Skipping candidate ${candidate.email}: no resume URL`);
                ...
                console.log(`[LocalFile OCR] Processing resume for: ${candidate.email}`);
                console.log(`[LocalFile OCR] Resume URL: ${candidate.resumeUrl}`);
                ...
                if (resumeText && resumeText.length > 50) {
                    ...
                    await candidate.save();
                    successCount++;
                    console.log(`[LocalFile OCR] Successfully extracted text for: ${candidate.email}`);
                } else {
                    candidate.isResumeScanned = true;
                    candidate.resumeSummary = resumeText || "Unable to extract text from resume";
                    await candidate.save();
                    failedCount++;
                    console.log(`[LocalFile OCR] Minimal text extracted for: ${candidate.email}`);
                }

            } catch (candidateError) {
                console.error(`[LocalFile OCR] Error processing candidate ${candidate.email}:`, candidateError.message);
                candidate.isResumeScanned = true;
        ...
        interview.status = "localfile_ocr_complete";
        interview.logs.push({
        ...
        // 5️⃣ Cleanup: Delete the temporary file if configured
        localFileData = await LocalFileData.findOne({ interview: interviewId });
        if (localFileData && localFileData.cleanupFile && localFileData.serverFilePath) {
            try {
                if (fs.existsSync(localFileData.serverFilePath)) {
                    fs.unlinkSync(localFileData.serverFilePath);
                    console.log(`[LocalFile OCR] Cleaned up temporary file: ${localFileData.serverFilePath}`);
                    localFileData.logs.push({ message: "Temporary file cleaned up", level: "info" });
                    localFileData.serverFilePath = null; // Clear the path
                    await localFileData.save();
                }
            } catch (cleanupError) {
                console.warn(`[LocalFile OCR] Failed to cleanup file: ${cleanupError.message}`);
            }
        }
        ...
        console.log(`[LocalFile OCR] Completed for interview: ${interviewId}`);
        console.log(`[LocalFile OCR] Success: ${successCount}, Failed: ${failedCount}`);

    } catch (error) {
        console.error("[LocalFile OCR] Error:", error);

        // Update interview status
        if (interview) {
            interview.currentStatus = "FAILED";
            interview.logs.push({
                message: `LocalFile OCR failed: ${error.message}`,
                level: "error"
            });
            await interview.save();
